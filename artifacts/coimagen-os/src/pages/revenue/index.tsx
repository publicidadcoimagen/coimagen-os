...
const CURRENCY_LINE_COLOR: Record<string, string> = { MXN: "hsl(var(--primary))", USD: "#22c55e" };
const FALLBACK_LINE_COLORS = ["#a855f7", "#f59e0b", "#ef4444"];

// Reshapes [{month, mrrByCurrency: [{currency, amount}]}] into one row per
// month with a numeric column per currency (recharts needs a flat key per
// series, not a nested array) — e.g. {month: "2026-09", MXN: 3000, USD: 300}.
function flattenTrendByCurrency(trend: { month: string; mrrByCurrency: { currency: string; amount: number }[] }[] | undefined) {
  const currencies = new Set<string>();
  const rows = (trend ?? []).map((point) => {
    const row: Record<string, string | number> = { month: point.month };
    for (const { currency, amount } of point.mrrByCurrency) { row[currency] = amount; currencies.add(currency); }
    return row;
  });
  return { rows, currencies: [...currencies] };
}
...
export function Revenue() {
  ...
  const { rows: trendRows, currencies: trendCurrencies } = flattenTrendByCurrency(mrrTrend);

  const STATS = [
    { label: "MRR", value: formatCurrencyBreakdown(summary?.mrrByCurrency), color: "text-emerald-400" },
    { label: "ARR", value: formatCurrencyBreakdown(summary?.arrByCurrency), color: "text-emerald-300" },
    ...
    { label: "Alto ticket (total)", value: formatCurrencyBreakdown(summary?.highTicketTotalByCurrency), color: "text-violet-300" },
  ];
            ...
            {STATS.map((s) => (
              <Card key={s.label}>
                <CardContent className="pt-4 pb-4">
                  ...
                  <div className={`text-sm font-bold leading-snug ${s.color}`}>{s.value}</div>
                </CardContent>
              </Card>
            ))}
          </div>
            ...
            <CardContent className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trendRows} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  ...
                  {/* One line per currency actually present — never a single blended MRR line (2026-09-22 currency audit). */}
                  <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", borderColor: "hsl(var(--border))", fontSize: 12 }} formatter={(v: number, name: string) => [formatCurrency(v, name), name]} />
                  {trendCurrencies.length > 1 && <Legend wrapperStyle={{ fontSize: 11 }} />}
                  {trendCurrencies.map((currency, i) => (
                    <Line key={currency} type="monotone" dataKey={currency} name={currency} stroke={CURRENCY_LINE_COLOR[currency] ?? FALLBACK_LINE_COLORS[i % FALLBACK_LINE_COLORS.length]} strokeWidth={2} dot={{ r: 3 }} connectNulls />
                  ))}
                </LineChart>
              </ResponsiveContainer>
            </CardContent>
                ...
                <div className="space-y-2">
                  {highTicket.map((inv) => (
                    <div key={inv.id} className="flex items-center justify-between text-sm border-b border-border/30 pb-2">
                      ...
                      <span className="font-bold text-emerald-400">{formatCurrency(inv.amount, inv.currency)}</span>
                    </div>
                  ))}
