    ...
    <ClientRoomLayout slug={slug}>
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{t.documents.contractsAndAgreements}</p>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 border border-dashed border-border/50 rounded-xl gap-3 text-muted-foreground">
              ...
              <p className="text-sm">{t.documents.emptyMsg}</p>
            </div>
          ) : (
            docs.map((doc) => (
                ...
                <CardContent className="p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[10px] text-muted-foreground">{t.documents.types[doc.type] ?? doc.type}</span>
                      {doc.signedAt && <Badge variant="outline" className="text-[9px] py-0 bg-green-400/10 text-green-400 border-green-400/30">{t.documents.signed}</Badge>}
                      <span className="text-[10px] text-muted-foreground">{new Date(doc.createdAt).toLocaleDateString(lang === "en" ? "en-US" : "es-MX")}</span>
                    </div>
                  </div>
                  <Badge variant="outline" className="text-[9px] py-0 flex-shrink-0 gap-1 cursor-not-allowed opacity-50">
                    <ExternalLink className="h-2.5 w-2.5" />{t.documents.pdfComingSoon}
                  </Badge>
                </CardContent>
              </Card>
            ))
        ...
        <Card className="border-border/30 bg-muted/5">
          <CardContent className="p-3 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-medium">{t.documents.googleDrive}</p>
              <p className="text-[10px] text-muted-foreground">{t.documents.driveDesc}</p>
            </div>
            <Badge variant="outline" className="text-[9px] py-0 flex-shrink-0">{t.common.comingSoon}</Badge>
          </CardContent>
        </Card>
      </div>
    </ClientRoomLayout>
